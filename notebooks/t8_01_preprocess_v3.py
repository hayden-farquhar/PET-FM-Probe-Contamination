...
# %% [markdown]
# # T8 Notebook 1/2 v3: Download & Preprocess Lung-PET-CT-Dx (SUV-converted)
#
# **PET-FM-Bench** | DOI: [10.17605/OSF.IO/DQ2JA](https://doi.org/10.17605/OSF.IO/DQ2JA)
#
# **Runtime:** CPU | **Internet:** On | **Time:** ~30–40 min
#
...
# - Adds size-filter to drop placeholder series (<1 MB)
# - Adds dose-sanity guard (skips series with dose <10 MBq)
# - Adds SUV QC log per session (SUVmax, scanner, etc.)
#
# **Why:** v1's dcm2niix output was in Bq/mL, which overflowed float16 patches
# (max 65504) and produced inf/NaN for ~37% of T8 sessions, breaking the 3D
# embedding path. Diagnosed in `00_diagnostic.py`. Fix validated in
# `01_suv_smoke_test.py` (T8 PASS clean, SUVmax 12.7).
#
# **Output:** `pet-fm-bench-t8-patches-v3` (do NOT overwrite v1 — preserves
# audit trail).
...
# Sanity guards (per smoke-test + T7 investigation findings)
MIN_SERIES_SIZE_MB = 1.0   # drop placeholder series (e.g., T7 had 35 KB "PET" series)
MIN_SLICE_COUNT = 50       # real PET has hundreds of slices
MIN_DOSE_MBQ = 10.0        # malformed dose tags read as 0.000407 MBq in T7
...
#
# Source: (companion project) `src/preprocess/suv_conversion.py`. Validated 2026-04-26 with
# max relative difference 0.00000% against Kinahan & Fletcher (2010) reference.
# Smoke-tested on this project data 2026-04-26 (T4/T6/T8 PASS, T7 PASS on representative
# series after edge-case characterisation).
#
# Critical: handles per-slice rescale (Siemens variable-rescale) correctly.
# Naive single-slice rescale produces 640% errors on Siemens scanners.
...
# %% [markdown]
# ## 4. Query TCIA and select PET series

# %%
print("Querying TCIA...")
all_series = nbia.getSeries(collection="Lung-PET-CT-Dx", format="df")
pet_series = all_series[all_series["Modality"] == "PT"].copy()
print(f"PET series found: {len(pet_series)} from {pet_series['PatientID'].nunique()} patients")

# Apply size filter (drops placeholder/non-imaging series)
pet_series["FileSize_MB"] = pet_series["FileSize"] / 1e6
n_before = len(pet_series)
pet_series = pet_series[pet_series["FileSize_MB"] >= MIN_SERIES_SIZE_MB].copy()
n_filtered = n_before - len(pet_series)
print(f"After size filter (>= {MIN_SERIES_SIZE_MB} MB): {len(pet_series)} series "
      f"({n_filtered} dropped as placeholders)")

# Subtype labels
pet_series["subtype"] = pet_series["PatientID"].apply(
    lambda x: SUBTYPE_MAP.get(x.split("-")[1][0], "unknown")
)
print(f"Subtypes: {pet_series.groupby('subtype')['PatientID'].nunique().to_dict()}")

# %% [markdown]
# ## 5. Download PET DICOM from TCIA

# %%
series_uids = pet_series["SeriesInstanceUID"].tolist()
print(f"Downloading {len(series_uids)} PET series "
      f"({pet_series['FileSize'].sum()/1e9:.1f} GB)...")
...
# %% [markdown]
# ## 6. Build manifest from DICOM directories
#
# Each series gets its own directory; we map UID → PatientID → DICOM dir.
...
# T8 has one PET series per patient — pick the largest if multiple ended up here
manifest = manifest.sort_values(["patient_id", "n_slices"], ascending=[True, False])
manifest = manifest.drop_duplicates(subset=["patient_id"], keep="first").reset_index(drop=True)
...
# %% [markdown]
# ## 8. Process all patients (DICOM → SUV → patches)
...
for _, row in tqdm(manifest.iterrows(), total=len(manifest), desc="Processing"):
    ...
    if pd.notna(row["skip_reason"]):
        log.append({"patient_id": pid, "status": f"skipped: {row['skip_reason']}"})
        continue

    try:
        # Get a sample DICOM file for metadata
        dcm_files = list(Path(dcm_dir).glob("*.dcm"))
        if not dcm_files:
            log.append({"patient_id": pid, "status": "no_dcm_files"})
            continue

        # Extract metadata + dose sanity check
        meta = extract_pet_metadata(str(dcm_files[0]))
        dose_mbq = meta.injected_dose_bq / 1e6
        if dose_mbq < MIN_DOSE_MBQ:
            log.append({
                "patient_id": pid,
                "status": f"skipped: dose {dose_mbq:.4f} MBq < {MIN_DOSE_MBQ} (malformed)",
        ...
        if not np.isfinite(pet).all():
            log.append({"patient_id": pid, "status": "non_finite_suv",
                        "manufacturer": meta.manufacturer})
            continue
        if suv_max > 200:
            # Plausible but unusual (extravasation, hot bladder, brain)
            print(f"  [QC FLAG] {pid}: SUVmax={suv_max:.1f} (unusually high)")

        # 3D patches — float16 is now safe, SUV values are 0-50ish
        ...
        log.append({"patient_id": pid, "status": f"error: {type(e).__name__}: {e}"})
...
print("=== T8 v3 SUV-converted Preprocessing QC ===")
...
if len(ok) > 0:
    ...
    print(f"  Patches per patient: {ok['n_patches'].astype(int).mean():.1f}")

    # Sanity: SUVmax should be in 1-100 range now (vs 50000+ in v1)
    n_high_suv = (ok["suv_max"] > 100).sum()
    n_extreme_suv = (ok["suv_max"] > 1000).sum()
    print(f"\n  SUVmax > 100: {n_high_suv} (high but plausible — extravasation/bladder)")
    print(f"  SUVmax > 1000: {n_extreme_suv} (should be 0; if not, SUV conversion failed)")

    if n_extreme_suv > 0:
        print(f"\n⚠ {n_extreme_suv} patients have SUVmax > 1000 — investigate manifest/manufacturer column")
...
# Output → **"New Dataset"** → `pet-fm-bench-t8-patches-v3`
# (do NOT overwrite v1 — preserves audit trail).
