...
# %% [markdown]
# # T7 Notebook 2/2: Extract FM Embeddings (ACRIN-NSCLC-FDG-PET)
#
# **PET-FM-Bench** | DOI: [10.17605/OSF.IO/DQ2JA](https://doi.org/10.17605/OSF.IO/DQ2JA)
#
# **Runtime:** GPU T4 | **Internet:** On | **Time:** ~2–3 hours
#
# **Input dataset:** `haydenfarquhar/pet-fm-bench-t7-patches`
#
# **Task:** T7 — FDG response prediction. Multiple timepoints per patient
# (pre/post chemoradiation). Embeddings saved per scan.
...
# %%
# Prefer v3 dataset if attached; falls back to v1 or any pet-fm-bench-t7* dataset.
# rglob handles Kaggle's variable mount depth (/kaggle/input/<ds>/ vs /kaggle/input/datasets/<user>/<ds>/).
V3_CANDIDATES = list(Path("/kaggle/input").rglob("pet-fm-bench-t7-patches-v3"))
INPUT_CANDIDATES = V3_CANDIDATES or list(Path("/kaggle/input").glob("pet-fm-bench-t7*"))
...
PATCH_DIR = INPUT_CANDIDATES[0] if INPUT_CANDIDATES else Path("/kaggle/input/pet-fm-bench-t7-patches")

# Find manifest
manifest_path = list(PATCH_DIR.rglob("manifest.csv"))[0]
PATCH_DIR = manifest_path.parent
manifest = pd.read_csv(manifest_path)
print(f"\nManifest (raw): {len(manifest)} scans from {manifest['patient_id'].nunique()} patients")

# File-existence filter: t7_01 writes manifest before per-scan loop, so
# failed/skipped scans stay in manifest but have no patch files.
def _has_files(r):
...
print(f"Manifest (filtered): {len(manifest)} scans from {manifest['patient_id'].nunique()} patients")
print(f"Dropped {dropped_n} scans with missing patch files")

EMBED_DIR = Path("/kaggle/working/embeddings")
EMBED_DIR.mkdir(parents=True, exist_ok=True)

# Save scan metadata alongside embeddings (post-filter so labels match embeddings)
manifest[["patient_id", "study_index", "is_baseline"]].to_csv(
    EMBED_DIR / "t7_labels.csv", index=False
)
...
# %%
def load_patches_3d(patient_id, scan_id):
    path = PATCH_DIR / "patches_3d" / patient_id / scan_id / "patches.npz"
    data = np.load(path)
    return data["patches"].astype(np.float32), data["positions"]


def load_mips(patient_id, scan_id):
    path = PATCH_DIR / "mips_2d" / patient_id / scan_id / "mips.npz"
    data = np.load(path)
...
# Filter manifest to only scans with preprocessed data
valid_rows = []
for _, row in manifest.iterrows():
    pid = row["patient_id"]
    scan_id = f"study_{row['study_index']}"
    has_patches = (PATCH_DIR / "patches_3d" / pid / scan_id / "patches.npz").exists()
    has_mips = (PATCH_DIR / "mips_2d" / pid / scan_id / "mips.npz").exists()
    if has_patches and has_mips:
        valid_rows.append(row)

manifest = pd.DataFrame(valid_rows).reset_index(drop=True)
print(f"Valid scans: {len(manifest)} from {manifest['patient_id'].nunique()} patients")
...
def run_dinov2(manifest):
    ...
    for _, r in tqdm(manifest.iterrows(), total=len(manifest), desc="DINOv2"):
        mips = load_mips(r["patient_id"], f"study_{r['study_index']}")
        for view, arr in mips.items():
            ...
            for layer_name, layer_idx in [("layer03", 3), ("layer06", 6),
                ...
                rows.append({"patient_id": r["patient_id"], "scan_id": f"study_{r['study_index']}", "view": view,
...
def run_fmcib(manifest):
    ...
    for _, r in tqdm(manifest.iterrows(), total=len(manifest), desc="FMCIB"):
        patches, positions = load_patches_3d(r["patient_id"], f"study_{r['study_index']}")
        ...
        rows.append({"patient_id": r["patient_id"], "scan_id": f"study_{r['study_index']}", "view": "volume",
...
# %%
print("\n" + "=" * 60)
print("T7 EMBEDDING EXTRACTION COMPLETE")
print("=" * 60)
...
print(f"Labels file: {(EMBED_DIR / 't7_labels.csv').stat().st_size / 1e3:.0f} KB")

# %% [markdown]
# ## 8. Save
#
# Commit with **"Save & Run All"**. Then save output as Kaggle Dataset:
# `pet-fm-bench-t7-embeddings`
