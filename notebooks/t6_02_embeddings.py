...
# %% [markdown]
# # T6 Notebook 2/2: Extract FM Embeddings (RIDER-Lung-PET-CT)
#
# **PET-FM-Bench** | DOI: [10.17605/OSF.IO/DQ2JA](https://doi.org/10.17605/OSF.IO/DQ2JA)
#
# **Runtime:** GPU T4 | **Internet:** On | **Time:** ~1–2 hours
#
# **Input dataset:** `haydenfarquhar/pet-fm-bench-t6-patches`
#
# **Task:** T6 — Cancer-patient test-retest embedding stability.
# Embeddings are saved with session labels for computing test-retest CCC.
...
# Prefer v3 dataset if attached; falls back to v1 or any pet-fm-bench-t6* dataset.
# rglob handles Kaggle's variable mount depth (/kaggle/input/<ds>/ vs /kaggle/input/datasets/<user>/<ds>/).
V3_CANDIDATES = list(Path("/kaggle/input").rglob("pet-fm-bench-t6-patches-v3"))
INPUT_CANDIDATES = V3_CANDIDATES or list(Path("/kaggle/input").glob("pet-fm-bench-t6*"))
...
PATCH_DIR = INPUT_CANDIDATES[0] if INPUT_CANDIDATES else Path("/kaggle/input/pet-fm-bench-t6-patches")
print(f"\nUsing: {PATCH_DIR}")
...
print(f"Manifest (raw): {len(manifest)} sessions, {manifest['patient_id'].nunique()} patients")

# File-existence filter: t6_01 writes the manifest BEFORE the per-session loop,
# so failed/skipped sessions stay in the manifest but have no patch files.
# Drop rows where mips_2d/<pid>/<sess>/mips.npz or patches_3d/<pid>/<sess>/patches.npz
# don't exist on disk.
def _has_files(r):
    mips = PATCH_DIR / "mips_2d" / r["patient_id"] / r["session"] / "mips.npz"
    pat3 = PATCH_DIR / "patches_3d" / r["patient_id"] / r["session"] / "patches.npz"
    return mips.exists() and pat3.exists()

mask = manifest.apply(_has_files, axis=1)
dropped_n = (~mask).sum()
manifest = manifest[mask].reset_index(drop=True)
print(f"Manifest (filtered): {len(manifest)} sessions, {manifest['patient_id'].nunique()} patients")
print(f"Dropped {dropped_n} sessions with missing patch files")
print(f"Test-retest patients: {manifest[manifest['is_retest_patient'] == True]['patient_id'].nunique()}")

# Output directory
EMBED_DIR = Path("/kaggle/working/embeddings")
EMBED_DIR.mkdir(parents=True, exist_ok=True)

# Save test-retest labels (post-filter so labels match actual embeddings)
manifest[["patient_id", "session", "study_index", "is_retest_patient"]].to_csv(
    EMBED_DIR / "t6_labels.csv", index=False
)
...
# %%
def load_patches_3d(patient_id, session):
    """Load preprocessed 3D patches for a session."""
    path = PATCH_DIR / "patches_3d" / patient_id / session / "patches.npz"
    data = np.load(path)
    return data["patches"].astype(np.float32), data["positions"]


def load_mips(patient_id, session):
    """Load preprocessed 2D MIPs for a session."""
    path = PATCH_DIR / "mips_2d" / patient_id / session / "mips.npz"
    data = np.load(path)
    return {
...
def run_dinov2(manifest):
    ...
    for _, r in tqdm(manifest.iterrows(), total=len(manifest), desc="DINOv2"):
        mips = load_mips(r["patient_id"], r["session"])
        for view, arr in mips.items():
            ...
            for layer_name, layer_idx in [("layer03", 3), ("layer06", 6),
                ...
                rows.append({
                    "patient_id": r["patient_id"], "session": r["session"],
...
def run_fmcib(manifest):
    ...
    for _, r in tqdm(manifest.iterrows(), total=len(manifest), desc="FMCIB"):
        patches, positions = load_patches_3d(r["patient_id"], r["session"])
...
# ## 8. Save
#
# Commit with **"Save & Run All"**.
# Then save the output as Kaggle Dataset: `pet-fm-bench-t6-embeddings`
