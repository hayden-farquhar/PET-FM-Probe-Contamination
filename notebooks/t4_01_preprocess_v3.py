...
# %% [markdown]
# # T4 Notebook 1/2 v3: Download & Preprocess NSCLC-Radiogenomics (SUV-converted)
#
# **PET-FM-Bench** | DOI: [10.17605/OSF.IO/DQ2JA](https://doi.org/10.17605/OSF.IO/DQ2JA)
#
# **Runtime:** CPU | **Internet:** On | **Time:** ~1.5–2.5 hours
#
...
# - Adds size filter (≥1 MB) to drop placeholder PET series
# - Adds dose sanity guard (skip series with dose <10 MBq)
# - Skips CT/SEG download — PET only (saves ~15 GB and ~30 min)
# - Preserves v2 survival logic: `time_to_death` populated for ALL patients
#   (alive patients get `Last Known Alive − CT Date`)
#
# **Why:** v1/v2 had **86% NaN dropout** on the 3D embedding path because
# dcm2niix output PET in Bq/mL, which overflowed float16 patches.
# Diagnosed in `00_diagnostic.py` (T4: 70.6% sessions with inf/NaN).
# T8 v3 validated the new pipeline 133/133 with 0 dropouts.
#
# **Output:** `pet-fm-bench-t4-patches-v3` (do NOT overwrite v1).
...
# %% [markdown]
# ## 3. Download clinical data (survival endpoints)
...
# %% [markdown]
# ## 5. Query TCIA and select primary PET series per patient
#
# Filtering strategy preserved from v1:
# - Modality == "PT"
# - Exclude NAC (non-attenuation-corrected) — we want AC PET only
# - Exclude reformatted views (COR / SAG / MIP) — we want the 3D AC volume
# - Pick the series with most slices per patient
# - Apply size filter (drops placeholder series)
...
# Size filter
pet_primary["FileSize_MB"] = pet_primary["FileSize"] / 1e6
n_before_size = len(pet_primary)
pet_primary = pet_primary[pet_primary["FileSize_MB"] >= MIN_SERIES_SIZE_MB].copy()
print(f"After size filter (>= {MIN_SERIES_SIZE_MB} MB): "
      f"{len(pet_primary)} series ({n_before_size - len(pet_primary)} dropped as placeholders)")

# Pick largest series per patient (highest ImageCount)
...
print(f"\nSelected PET: {len(pet_best)} patients ({pet_best['FileSize'].sum()/1e9:.1f} GB)")

# %% [markdown]
# ## 6. Download PET DICOM (PET only — skipping CT/SEG saves ~15 GB)

# %%
series_uids = pet_best["SeriesInstanceUID"].tolist()
print(f"Downloading {len(series_uids)} PET series ({pet_best['FileSize'].sum()/1e9:.1f} GB)...")
print("This will take ~30-60 min depending on network.")
...
# %% [markdown]
# ## 7. Build processing manifest with survival labels
#
# Survival logic (preserves v2 fix):
# - Dead patients: time_to_death = "Time to Death (days)" from clinical CSV
# - Alive patients: time_to_death = days from "CT Date" to "Date of Last Known Alive"
#   (computed from shifted dates — differences preserved by TCIA's de-identification)

# %%
uid_to_patient = dict(zip(pet_best["SeriesInstanceUID"], pet_best["PatientID"]))
...
print(f"  With survival time: {manifest['time_to_death'].notna().sum()}")
manifest.to_csv(PATCH_DIR / "manifest.csv", index=False)

# %% [markdown]
# ## 8. Preprocessing functions (identical to T8 v3)
...
# %% [markdown]
# ## 9. Process all patients (DICOM → SUV → patches)
...
print("=== T4 v3 SUV-converted Preprocessing QC ===")
...
# Survival data integrity (v2 fix preservation)
ok_with_surv = ok.merge(manifest[["patient_id", "event", "survival_status", "time_to_death"]], on="patient_id")
print(f"\nSurvival endpoint:")
print(f"  Events (deaths): {ok_with_surv['event'].sum()}")
print(f"  Alive with follow-up time: "
      f"{ok_with_surv[ok_with_surv['event']==0]['time_to_death'].notna().sum()}/"
      f"{(ok_with_surv['event']==0).sum()}")
print(f"  Dead with time-to-death: "
      f"{ok_with_surv[ok_with_surv['event']==1]['time_to_death'].notna().sum()}/"
      f"{(ok_with_surv['event']==1).sum()}")
...
# Output → **"New Dataset"** → `pet-fm-bench-t4-patches-v3`.
