...
# %% [markdown]
# # T4 Notebook 2/2: Extract FM Embeddings (NSCLC-Radiogenomics)
#
# **PET-FM-Bench** | DOI: [10.17605/OSF.IO/DQ2JA](https://doi.org/10.17605/OSF.IO/DQ2JA)
#
# **Runtime:** GPU T4 | **Internet:** On | **Time:** ~1–2 hours
#
# **Input dataset:** `haydenfarquhar/pet-fm-bench-t4-patches`
#
# **Task:** T4 — NSCLC survival prediction. Embeddings are saved with
# patient-level survival labels for downstream CoxPH / gradient-boosted
# survival probes.
...
# %%
# Prefer v3 dataset if attached; falls back to v1 or any pet-fm-bench-t4* dataset.
# rglob handles Kaggle's variable mount depth (/kaggle/input/<ds>/ vs /kaggle/input/datasets/<user>/<ds>/).
V3_CANDIDATES = list(Path("/kaggle/input").rglob("pet-fm-bench-t4-patches-v3"))
INPUT_CANDIDATES = V3_CANDIDATES or list(Path("/kaggle/input").glob("pet-fm-bench-t4*"))
...
PATCH_DIR = INPUT_CANDIDATES[0] if INPUT_CANDIDATES else Path("/kaggle/input/pet-fm-bench-t4-patches")
...
# File-existence filter: t4_01 writes manifest before per-patient loop, so
...
# Save clinical labels alongside embeddings (post-filter so labels match embeddings)
manifest[["patient_id", "survival_status", "time_to_death", "event"]].to_csv(
    EMBED_DIR / "t4_labels.csv", index=False
)
...
# %%
print("\n" + "=" * 60)
print("T4 EMBEDDING EXTRACTION COMPLETE")
print("=" * 60)
...
print(f"Labels file: {(EMBED_DIR / 't4_labels.csv').stat().st_size / 1e3:.0f} KB")

# %% [markdown]
# ## 8. Save
#
# Commit with **"Save & Run All"**. Then save output as Kaggle Dataset:
# `haydenfarquhar/pet-fm-bench-t4-embeddings`
